...
export class Paper {
  ...
  constructor (el: HTMLElement, panel: any, lang?: string, stencilEl?: HTMLElement, inspectorEl?: HTMLElement, toolbarEl?: HTMLElement) {
    ...
    this.paper = new joint.dia.Paper({
      ...
      guard: function (evt) {
        return Boolean(window.online) && (evt.target instanceof HTMLInputElement || evt.target instanceof HTMLSelectElement || evt.target instanceof HTMLButtonElement)
      }
    })
    ...
    if (!window.online) {
      ...
      const StencilConfig = getStencilConfig(this.paper, this.snaplines, lang || '')
  ...
  initSelectionEvents (inspectorEl?:HTMLElement) {
    ...
    this.selection.collection.on('reset add remove', (collection: Backbone.Collection) => {
      ...
      // 添加inspector
      const inspectorConfig = getInspectorConfig(collection, this.bindOptions)
