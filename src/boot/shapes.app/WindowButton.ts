...
export class WindowButton extends joint.dia.Element {
  defaults () {
    return {
      ...super.defaults,
      type: 'app.WindowButton',
...
export const WindowButtonView = joint.dia.ElementView.extend({
      ...
      children: [
        {
          attributes: {
            overflow: 'hidden',
            style: `width: 100%; height: 100%; background: ${model.attributes.background}`,
  ...
  onClick () {
    // const bindTarget = get(this, 'model.attributes.window.onClick')
    // if (bindTarget) window.open(`/#/screen?name=${bindTarget}`, '', 'width=600,height=400,left=200,top=200')
    if (window.online) {
      const bindTarget = get(this, 'model.attributes.window.onClick')
      if (bindTarget) window.open(`/hmi/screen=${window.screenName}&window=${bindTarget}#/screen`, '', 'width=600,height=400,left=200,top=200')
